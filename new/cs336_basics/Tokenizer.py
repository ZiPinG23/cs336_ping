...
class Tokenizer:
    ...
    def from_files(
        ...
        def bytes_to_str(b: bytes) -> str:
            ...
            s = ""
            for byte in b:
                s.join(byte_to_uni[byte])
            return s

        def str_to_bytes(s: str) -> bytes:
            byte_to_uni = bytes_to_unicode()
            byte_decode = {v: k for k, v in byte_to_uni.items()}
            ans = bytearray()
            for c in s:
                ans.append(byte_decode[c])
            return ans
    ...
    def pre_tokenization(
        ...
        split_texts = (
            re.split(split_pattern, input) if len(escaped_tokens) != 0 else [input]
        )  # 得到分割后的文本，格式为list
        pre_tokens = []
    ...
    def merge(
        self, pre_token: tuple[bytes], ranked_merges: dict[bytes, int]
    ) -> tuple[bytes]:
        while True:
            ...
            if best_pair == None: #说明已经没有可以合并的词语了
                break
    ...
    def merge_pre_tokens(self, pre_tokens: list[tuple[bytes]]) -> list[tuple[bytes]]:
        ...
        ranked_merges = {
            bytes1 + bytes2: idx for idx, (bytes1, bytes2) in enumerate(self.merges)
        }
    ...
    def encode(self, text: str) -> list[int]:
        ...
        merge_tokens = self.merge_pre_tokens(pre_tokens)
        joined_tokens = []
        for merge_token in merge_tokens:
            for b in merge_token:
